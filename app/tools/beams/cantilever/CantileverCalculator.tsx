...
export default function CantileverCalculator() {
  ...
  // ── Deflection limit ─────────────────────────────────────────────────────
  const [deflectionNStr, setDeflectionNStr] = useState<string>('200');
  const deflectionN = (() => {
  ...
  const [formulaSteps, setFormulaSteps] = useState<ReturnType<typeof buildCantileverFormulaSteps>>([]);
  ...
  return (
    <div>
      ...
      <form className="beam-form" onSubmit={handleCalculate} noValidate>
        ...
        {/* ② Span */}
        <section className="beam-section">
          <h2 className="beam-section-title">② スパン（片持ち長さ）</h2>
          <div className="beam-row">
            <div className="form-group" style={{ flex: '1 1 200px' }}>
              <label htmlFor="L">
            ...
            <div className="form-group" style={{ flex: '1 1 220px' }}>
              ...
              <div className="input-with-unit">
                <input
                  ...
                  min="0.001"
                  step="any"
                  placeholder={loadUnit === 'kg' ? '例: 500' : '例: 4.9'}
                  value={loadValue}
          ...
          <div className="beam-row" style={{ alignItems: 'flex-start', gap: '1rem' }}>
            <div className="form-group" style={{ flex: '0 0 auto', maxWidth: 260 }}>
              <label htmlFor="deflectionN">
                分母 N（L / N）
                <span className="unit-label" style={{ marginLeft: 6 }}>例: 200 → L/200</span>
              </label>
            ...
            <div className="beam-deflection-ref">
              <p className="beam-ref-title">📖 参考：片持ち梁の許容たわみ目安</p>
              <ul className="beam-ref-text-list">
                <li><strong>L/150〜L/200</strong>：一般的な庇・ブラケットの目安</li>
                <li><strong>L/250</strong>：仕上げ材のある片持ち梁（変形による影響を考慮）</li>
                <li><strong>L/300〜L/400</strong>：精密機器架台・先端にセンサー等がある用途</li>
              </ul>
              <p className="beam-ref-note">
                ※ 片持ち梁は単純梁よりたわみが大きいため、許容基準をやや緩めに設定することが一般的です。
      ...
      {result && (
        <div className="results" style={{ marginTop: '2rem' }}>
          ...
          <p className="result-meta">
            {loadCase === 'center' ? '先端集中荷重' : '等分布荷重（総荷重入力）'}
          ...
          <div className="result-cards">
            <div className="result-card">
              <p className="result-label">最大曲げモーメント M_max（固定端）</p>
            ...
            <div className={`result-card${result.deflectionOK ? ' result-card--ok' : ' result-card--ng'}`}>
              <p className="result-label">最大たわみ δ_max（自由端）</p>
              <p className="result-value" style={{ fontSize: '1.25rem' }}>
      ...
      <div className="beam-notes-section">
        <h3>注記</h3>
        <ul>
          <li>片持ち梁は固定端の剛性が計算通りであることが前提です。実際の溶接・ボルト接合では固定度が100%でない場合があります。</li>
