/**
 * Formula step generator for simply-supported beam calculations.
...
export function buildBeamFormulaSteps(params: {
  loadCase: LoadCase;
  loadKN: number;          // kN
  L_mm: number;
  E_GPa: number;
  ...
  if (loadCase === 'center') {
    steps.push({
      label: '最大たわみ δ_max',
      expr: `δ_max = P × L³ / (48 × E × I)\n      = ${fmt(P_N, 0)} × ${fmt(L_mm, 0)}³ / (48 × ${fmt(E_MPa, 0)} × ${fmt(I_mm4, 0)})\n      = ${fmt(result.deltaMax, 3)} mm`,
    });
  } else {
    const w = result.w_N_per_mm!;
    steps.push({
      label: '最大たわみ δ_max',
      expr: `δ_max = 5 × w × L⁴ / (384 × E × I)\n      = 5 × ${fmt(w, 6)} × ${fmt(L_mm, 0)}⁴ / (384 × ${fmt(E_MPa, 0)} × ${fmt(I_mm4, 0)})\n      = ${fmt(result.deltaMax, 3)} mm`,
    });
  }
  steps.push({
    label: '許容たわみ δ_allow',
